...
export function ContactForm() {
  ...
  return (
    <form action={formAction} className="max-w-md space-y-4">
      ...
      {state.message && (
        <p className={cn('text-sm', state.success ? 'text-green-600' : 'text-red-600')} role="status">
          {state.message}
        </p>
      )}

      <Button type="submit" className="bg-brand-primary hover:bg-brand-primary/90" disabled={state.success}>
