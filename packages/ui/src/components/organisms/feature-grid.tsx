  ...
  ({ className, icon, title, description, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'rounded-lg bg-background-primary p-6 text-center shadow-sm border border-border-primary',
          className
        )}
        {...props}
      >
...
const FeatureGrid = React.forwardRef<HTMLDivElement, FeatureGridProps>(
  ({ className, asChild = false, columns, gap, features, centered = true, children, ...props }, ref) => {
    const Comp = asChild ? Slot : 'div'

    return (
      <Comp
        ref={ref}
        className={cn('grid', featureGridVariants({ columns, gap }), className)}
        {...props}
      >
