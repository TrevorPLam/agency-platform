...
function formatThemeTokens(tokens: Record<string, unknown>, outputReferences: boolean): string {
  return Object.entries(tokens)
    .map(([key, value]) => formatTokenValue(key, value as TokenValue, outputReferences, 2))
    .join('\n')
}

/** Recursively format a token tree for :root block. */
function formatRootVariables(tokens: Record<string, unknown>, outputReferences: boolean): string {
  return Object.entries(tokens)
    .map(([key, value]) => formatTokenValue(key, value as TokenValue, outputReferences, 2))
    .join('\n')
}
...
function formatTokenValue(
  ...
  if (
    ...
    return Object.entries(value)
      .map(([subKey, subValue]) =>
        formatTokenValue(`${key}-${subKey}`, subValue as TokenValue, outputReferences, indent)
      )
      .join('\n')
  }
      ...
      files: [
        {
          ...
          filter: (token: { path: string[] }) => token.path.includes('primitive'),
        },
      ],
    },
    'css/semantic': {
      ...
      files: [
        {
          ...
          filter: (token: { path: string[] }) => token.path.includes('semantic'),
          options: {
      ...
      files: [
        {
          ...
          format: 'css/root-variables',
          filter: (token: { path: string[] }) => token.path.includes('component'),
        },
      ],
    },
  },
