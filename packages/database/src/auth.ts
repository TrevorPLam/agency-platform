...
export interface CreateUserOptions {
  ...
  password?: string
  tenantId: TenantId
  /** Role within the tenant; stored in app_metadata and tenant_users. Defaults to 'member'. */
  role?: string
 ...
 * ```
 */
export async function createUserForTenant(options: CreateUserOptions): Promise<CreateUserResult> {
  const { email, password, tenantId, role = 'member', metadata, emailConfirm = false } = options
  ...
  try {
    ...
    const assigned = await assignUserToTenant(authData.user.id, tenantId, role)
    if (!assigned) {
 ...
 * ```
 */
export async function getUserTenants(userId: UserId): Promise<
  Array<{
    tenantId: TenantId
    role: string
    tenantName?: string
