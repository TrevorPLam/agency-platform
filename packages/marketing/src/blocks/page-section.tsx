...
export interface PageSectionProps
  extends React.HTMLAttributes<HTMLElement>, VariantProps<typeof pageSectionVariants> {
  asChild?: boolean
      ...
      >
        <div className={cn('container mx-auto px-4', centered && 'text-center')}>
          {heading || subheading || action ? (
            <div className="mb-12">
              {heading ? (
                <h2 className="text-text-primary mb-4 text-3xl font-bold md:text-4xl">{heading}</h2>
              ) : null}
              {subheading ? (
                <p className="text-text-secondary mb-8 text-lg md:text-xl">{subheading}</p>
              ) : null}
              {action ? (
                <div className={cn('flex gap-4', centered && 'justify-center')}>{action}</div>
              ) : null}
            </div>
          ) : null}
          {children}
