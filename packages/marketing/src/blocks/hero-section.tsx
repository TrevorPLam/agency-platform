...
export interface HeroSectionProps
  extends React.HTMLAttributes<HTMLElement>, VariantProps<typeof heroSectionVariants> {
  asChild?: boolean
  ...
  (
    ...
    return (
      <Comp
        ref={ref}
        className={cn(
          heroSectionVariants({ size, alignment: finalAlignment, background }),
          className
        )}
        {...props}
      >
        <div className="container mx-auto px-4">
          ...
          >
            {headline ? (
              <h1 className="text-text-primary mb-6 text-4xl font-bold md:text-5xl lg:text-6xl">
                {headline}
              </h1>
            ) : null}

            {subheadline ? (
              <p className="text-text-secondary mb-8 text-xl md:text-2xl lg:text-3xl">
                {subheadline}
              </p>
            ) : null}
